...
function myDash($scope, $mdToast, $http, $interval, $cookies) {
    ...
    $scope.tpl_authenticate = function () {
        ...
        $http.post(arr[0] + "//wap.tplinkcloud.com/", auth_obj).then(function mySuccess(response) {
            var now = new Date();
    ...
    $scope.tpl_refreshDevices = function () {
        ...
        $http.post(arr[0] + "//wap.tplinkcloud.com?token=" + $scope.tpl.token, request_obj).then(function mySuccess(response) {
    ...
    $scope.tpl = {};
    $scope.tpl.refresh_rate = 1; //check every 5 seconds. Set this as you fancy.
    ...
    $scope.onloadFun = function() {
      $scope.tpl_authenticate();
    }
    
    $scope.stateOn = function() {
    	$scope.tpl_setState("1", true);
    }
    
    $scope.stateOff = function() {
    	$scope.tpl_setState("1", false);
    }

    $scope.getState = function() {
         $scope.tpl_getState("1");
    }
}
